...
start_log()
for config in tqdm(best_param_candidates):
    print("params:", config)
    np.random.seed(random_seed)
    random.seed(random_seed)
    window_size, num_clusters, smoothing_size, order = get_params_from_string(config)
    ...
    #for i in range(1):
        ...
        sents_v = fsents_v[i]
        vectors = fvectors[i]
        sentence_beginnings = fsentence_beginnings[i]
        ...
        iou_full = eval_scenes(data["scenes"], scenes_pred)
        iou_all += iou_full
